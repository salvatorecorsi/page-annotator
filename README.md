...
- **API**: namespace REST `page-annotator`, rotta `/annotations/{post_id}` (GET/POST). L'SVG salvato è sanitizzato con `wp_kses` (la whitelist include `<path>`, `<g>` e `<image>`).
- **Immagini**: l'upload SVG nella Media Library è abilitato solo per chi ha `edit_posts` e ogni file SVG è sanitizzato in upload via `DOMDocument` (`includes/svg-upload.php`) — rimozione di `<script>`, attributi `on*`, `javascript:`/`vbscript:`, `<foreignObject>`, DOCTYPE/entità. I raster restano riferiti per URL.
- **Frontend**: `frontend.js` legge i dati via `wp_localize_script` e anima i path con GSAP + ScrollTrigger (caricati da CDN); le `<image>` non sono animate.
